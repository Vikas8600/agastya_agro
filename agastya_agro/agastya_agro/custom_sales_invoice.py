    ...
    if self.additional_discount == 0:
        ...
        for doc in self.items:
            ...
            if nrv:
                ...
                doc.nrv = nrv
                frappe.db.set_value("Sales Invoice Item",doc.name,"nrv",nrv)
                price_nrv.append(flt(nrv) * flt(doc.qty))
                
            if st:
                doc.stock_transfer = st
                frappe.db.set_value("Sales Invoice Item",doc.name,"stock_transfer",st)
        ...
        if sum(price_nrv) > self.grand_total and self.is_return == 0:
            frappe.throw("Grand total of sales invoice is less that NRV price")
